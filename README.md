...
- `DEBUG_PROMPT` — when set to a truthy value (for example, `1`), the server writes Gemini request stubs (`server/debug/gemini-request-*.json`) and error stubs (`server/debug/gemini-error-*.json`) for server-side debugging only.
